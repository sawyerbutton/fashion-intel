"""Highsnobiety RSS 爬虫：解析 RSS feed 获取潮流资讯。"""
...
logger = logging.getLogger("crawler.highsnobiety")

RSS_URL = "https://www.highsnobiety.com/feed/"


class HighsnobietyCrawler(BaseCrawler):
    platform = "highsnobiety"
    ...
    def search(self, keyword: str, max_notes: int = 20) -> list[dict]:
        ...
        logger.info("正在拉取 Highsnobiety RSS feed...")
        ...
        for entry in feed.entries:
            ...
            author = getattr(entry, "author", "Highsnobiety")
        ...
        logger.info("Highsnobiety 过滤完成，共 %d 条结果", len(results))
        return results
